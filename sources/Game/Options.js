...
export class Options
{
    ...
    {        
        if(this.game.rendering.renderer.backend.isWebGLBackend)
        {
            ...
            tooltip.innerHTML = /* html */`Seu navegador <strong>não é compatível</strong> com WebGPU, reduzindo o desempenho`
        }
    }
    ...
    setServer()
    {
        ...
        const update = (connected) =>
        {
            if(connected)
            {
                ...
                tooltip.innerHTML = /* html */`Recursos <strong>multijogador</strong> disponíveis`
            }
            else
            {
                element.classList.remove('is-success')
